...
Before we continue to the next step, we should read the syntax description of [*pairs*](https://stat.ethz.ch/R-manual/R-devel/library/graphics/html/pairs.html)function.
...
There are at least three R packages provided functions to plot corrgram, they are:
...
In this hands-on exercise, we will focus on corrplot. However, we can also explore the other two packages too.

Before getting started, we are to read [An Introduction to corrplot Package](https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html) in order to gain basic understanding of **corrplot** package.
...
![With corrplot package, we can use the *cor.mtest()* to compute the p-values and confidence interval for each pair of variables.](images/clipboard-1624339769.png)
